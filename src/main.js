...
(function() {
    ...
    // Solo ejecutar en el iframe del juego
    ...
    const Logger = {
        ...
        styles: {
            ...
            error: 'color: #ff0000; font-weight: bold; background: #220000; padding: 2px;',
            game: 'color: #ffa500; font-weight: bold;',
            python: 'color: #ffff00; font-weight: bold;'
        },
        log: function(msg, type = 'info') {
            console.log(`%c${this.prefix} ${msg}`, this.styles[type] || this.styles.info);
        },
        error: function(msg, err) {
            console.error(`%c${this.prefix} [ERROR] ${msg}`, this.styles.error, err || '');
        }
    };
    ...
    // --- VARIABLES GLOBALES ---
    let gameSocket = null;       // El socket de Socket.io (Juego)
    let pythonSocket = null;     // El WebSocket nativo (Python)
    let lastSetupData = null;    // Caché del evento setup
    const PYTHON_URL = "ws://localhost:8765";

    // --- 1. CONEXIÓN CON PYTHON ---
    function connectToPython() {
        pythonSocket = new WebSocket(PYTHON_URL);

        pythonSocket.onopen = () => {
            Logger.log("Conectado al Cerebro Python", 'success');
            notificarEstado("Python: Conectado");
            
            // Sincronizar configuración completa (UI -> Python)
            sendFullConfig();

            // Si tenemos datos de setup cacheados (por reconexión), enviarlos de nuevo
            if (lastSetupData) {
                Logger.log("Reenviando setup cacheado a Python...", 'info');
    ...
    function procesarOrdenPython(orden) {
        ...
        if (orden.action === "escribir_palabra") {
            ...
            Logger.log(`Python dice: Escribir "${palabra}"`, 'python');
            
            // Emitir al servidor del juego
            // 'setWord' es el evento estándar de BombParty, el true final suele ser "submit"
            gameSocket.emit('setWord', palabra, true);
        }

        if (orden.action === "teclear_texto") {
            // Simula escribir en el input sin enviar (false al final)
            const texto = orden.text;
            gameSocket.emit('setWord', texto, false);
        }

        if (orden.action === "unirse_juego") {
            Logger.log("Python dice: Unirse al juego", 'python');
            gameSocket.emit('joinRound');
        }
    }

    // --- 3. INTERCEPTAR SOCKET.IO (MONKEY PATCHING) ---
    // Guardamos la referencia original de window.io (si existe, o esperamos a que se defina)
    // En JKLM, window.io suele ser la función constructora del cliente socket.io

    // Definimos un getter/setter para capturar cuando el juego define 'io'
    ...
    Object.defineProperty(window, 'io', {
        ...
        set: function(newValue) {
            originalIo = function(...args) {
                Logger.log("El juego está creando un socket...", 'game');
                const socket = newValue(...args);
                
                // Guardamos la referencia al socket del juego
                gameSocket = socket;
                setupGameSocketListeners(socket);
                
                return socket;
            };
            // Copiar propiedades estáticas si las hay
    ...
    function setupGameSocketListeners(socket) {
        // Interceptamos todos los eventos entrantes
        const onevent = socket.onevent;
        socket.onevent = function(packet) {
            const args = packet.data || [];
            const eventName = args[0];
            const eventData = args[1]; // A veces es un objeto, a veces son múltiples argumentos

            // Reenviar eventos relevantes a Python
            if (pythonSocket && pythonSocket.readyState === WebSocket.OPEN) {
                // Lista COMPLETA de eventos que queremos monitorear
                ...
                if (eventosInteresantes.includes(eventName)) {
                    // Si hay más de un argumento de datos (ej: failWord(id, reason)), enviamos todo el array args
                    // quitando el nombre del evento.
                    let payload = eventData;
                    if (args.length > 2) {
                        payload = args.slice(1);
                    }

                    // CACHÉ: Guardar setup por si Python se desconecta
                    if (eventName === 'setup') {
                        lastSetupData = payload;
                        Logger.log("Setup capturado y cacheado.", 'info');
                    }
            ...
            // Ejecutar el comportamiento original del juego
            if (onevent) onevent.call(this, packet);
        };

        Logger.log("Listeners inyectados en el GameSocket.", 'success');
    ...
    function createUI() {
        ...
        // Separador
        body.appendChild(document.createElement('hr')).style = "border: 0; border-top: 1px solid #333; margin: 5px 0; width: 100%;";
        ...
        body.appendChild(createCheckbox("strategy_longest", "Maximizar Longitud", false));
        body.appendChild(createCheckbox("strategy_alphabet", "Maximizar Alfabeto", false));
        body.appendChild(createCheckbox("strategy_shortest", "Modo Pánico (Cortas)", false));
        ...
        body.appendChild(createNumberInput("minTypingDelay", "Min Typing Delay", 0.05));
        body.appendChild(createNumberInput("maxTypingDelay", "Max Typing Delay", 0.15));
        body.appendChild(createNumberInput("startDelayMin", "Start Delay Min", 0.5));
        body.appendChild(createNumberInput("startDelayMax", "Start Delay Max", 1.5));
        ...
        manualInputWrapper.style = "display: flex; flex-direction: column; gap: 2px; margin-bottom: 5px;";
        ...
        manualInput.style = "background: #222; color: #fff; border: 1px solid #004400; padding: 4px; border-radius: 4px; width: 100%; box-sizing: border-box; font-size: 11px;";
        
        // Intervalo para mantener foco forzadamente
        let focusInterval = null;
        
        // Al hacer foco: mantener foco activamente
        manualInput.onfocus = () => {
            // Forzar foco cada 100ms para evitar que el juego lo robe
        ...
        // Al perder foco: detener forzado de foco
        manualInput.onblur = () => {
            if (focusInterval) {
    ...
    function createNumberInput(id, labelText, defaultValue) {
        const wrapper = document.createElement('div');
        wrapper.style = "display: flex; flex-direction: column; gap: 2px; margin-bottom: 5px;";
        ...
        input.step = "0.01";
        input.style = "background: #222; color: #0f0; border: 1px solid #004400; padding: 2px 4px; border-radius: 4px; width: 100%; box-sizing: border-box;";
    ...
    function sendFullConfig() {
        ...
        const config = {};
        // Checkboxes
        ['active', 'autojoin', 'suicide', 'strategy_longest', 'strategy_alphabet', 'strategy_shortest'].forEach(id => {
            const el = document.getElementById("bot-cfg-" + id);
            if (el) config[id] = el.checked;
        });
        
        // Number inputs
        ['minTypingDelay', 'maxTypingDelay', 'startDelayMin', 'startDelayMax'].forEach(id => {
            const el = document.getElementById("bot-cfg-" + id);
        ...
        Logger.log("[CONFIG] Configuración completa sincronizada.", 'info');
    }
    ...
    function sendConfigUpdate(key, value) {
        if (pythonSocket && pythonSocket.readyState === WebSocket.OPEN) {
            ...
            Logger.log(`[CONFIG] Config enviada: ${key}=${value}`, 'info');
        }
    }

    function sendCustomMessage(text) {
        if (pythonSocket && pythonSocket.readyState === WebSocket.OPEN && text.trim().length > 0) {
            pythonSocket.send(JSON.stringify({
                event: "customMessage",
                data: text
            }));
            Logger.log(`[CHAT] Frase encolada: "${text}"`, 'info');
        }
    }

    // --- MODIFICAR notificarEstado PARA USAR LA NUEVA UI ---
    function notificarEstado(texto) {
        // Extraer estado simple para el color
    ...
    // Esperar a que el body exista para crear la UI
